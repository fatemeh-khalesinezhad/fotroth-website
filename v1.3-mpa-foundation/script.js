...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function setLanguage(lang) {
    ...
    if (typeof window.renderCollectiveCards === 'function') {
      window.renderCollectiveCards();
    }

    if (typeof window.initCollectionPage === 'function') {
      window.initCollectionPage();
    }

    if (typeof window.initFeaturedTeaser === 'function') {
      window.initFeaturedTeaser();
    }

    if (typeof window.highlightActiveNav === 'function') {
  ...
  if (cursorDot && cursorFollower) {
    ...
    function bindCursorTriggers() {
      document.querySelectorAll('.museum-hall-tile, .masonry-card, .collective-card, .btn-primary, .btn-gold, .btn-red, .journal-item, .featured-teaser-card, .sidebar-filter-btn').forEach(item => {
        item.addEventListener('mouseenter', () => {
          ...
          } else if (item.classList.contains('masonry-card') || item.classList.contains('featured-teaser-card')) {
          ...
          } else if (item.classList.contains('sidebar-filter-btn')) {
            if (cursorText) cursorText.textContent = currentLang === 'fa' ? 'انتخاب' : 'SELECT';
          } else {
  ...
  // ==========================================
  // 7B. ACTIVE NAVIGATION HIGHLIGHTER
  // ==========================================
  function highlightActiveNav() {
    ...
    const links = document.querySelectorAll('.main-nav .nav-link, .mobile-nav-links .mobile-nav-item');
    links.forEach(link => {
      const href = link.getAttribute('href');
      if (!href) return;
      const target = href.split('#')[0].replace('./', '');
      const isMatch = target === filename || (filename === 'index.html' && (target === '' || target === 'index.html'));
      if (isMatch) {
  ...
  // ==========================================
  // 7C. THE COLLECTION (MULTI-PAGE ARCHITECTURE)
  // Sticky left sidebar + dynamic right projects
  // ==========================================
  const collectionSidebarFilters = document.getElementById('collection-sidebar-filters');
  const collectionCuratorialBox = document.getElementById('collection-curatorial-box');
  const collectionMasonryGrid = document.getElementById('collection-masonry-grid');
  let activeCollectionCategory = 'all';

  function initCollectionPage() {
    if (!collectionSidebarFilters || !collectionMasonryGrid) return;

    const categories = store.getCategories();
    const allProjects = store.getAllProjects ? store.getAllProjects() : store.getProjects();
    const isFa = currentLang === 'fa';
    ...
    const filterItems = [
      {
        id: 'all',
        name: isFa ? 'همه شاخه‌ها' : 'ALL DISCIPLINES',
        count: allProjects.length,
        hall_num: 'WING I–VII'
      },
      ...categories.map((c, i) => ({
        id: c.id,
        name: isFa && c.name_fa ? c.name_fa : c.name,
        count: store.getProjects(c.id).length,
        hall_num: c.hall_num || `WING 0${i + 1}`
      }))
    ];

    collectionSidebarFilters.innerHTML = filterItems.map(item => `
      <button class="sidebar-filter-btn ${item.id === activeCollectionCategory ? 'active' : ''}" data-category-id="${item.id}" role="tab" aria-selected="${item.id === activeCollectionCategory}">
        <div class="filter-left-group">
          <span class="filter-hall-dot"></span>
          <span>${item.name}</span>
        </div>
    ...
    // Attach click events
    collectionSidebarFilters.querySelectorAll('.sidebar-filter-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        const catId = btn.getAttribute('data-category-id');
        activeCollectionCategory = catId;
        collectionSidebarFilters.querySelectorAll('.sidebar-filter-btn').forEach(b => b.classList.remove('active'));
        btn.classList.add('active');
        renderCollectionProjects(catId);
      });
    });

    renderCollectionProjects(activeCollectionCategory);
  }

  function renderCollectionProjects(catId) {
    if (!collectionMasonryGrid) return;
    const isFa = currentLang === 'fa';
    let projects = [];
    let curatorialData = null;

    if (catId === 'all') {
      projects = store.getAllProjects ? store.getAllProjects() : store.getProjects();
      curatorialData = {
        wing: isFa ? 'تمام تالارها // ۷ شاخه هنری' : 'ALL WINGS // 07 DISCIPLINES',
        title: isFa ? 'مجموعه جامع آثار فتروث' : 'The Complete Fotroth Archive',
        subtitle: isFa ? 'آرشیو آثار عکاسی فاخر، کمپین‌های بین‌المللی و ادیتوریال های‌فشن' : 'Complete exhibition archive of haute fashion, luxury commercial, and architectural works.',
        statement: isFa ? 'مجموعه‌ای گردآوری‌شده از تمامی تالارهای هنری تحت هدایت امیر مقدم، بازتاب‌دهنده مهندسی پیشرفته نورپردازی سایه‌روشن و استاندارد بدون مصالحه در ثبت جزئیات با دوربین‌های مدیوم‌فرمت.' : 'An expansive curatorial survey across seven visual wings directed by Amir Moghadam, combining Renaissance chiaroscuro lighting architecture with medium-format analog and digital capture.'
      };
    } else {
      const cat = store.getCategoryById(catId);
      projects = store.getProjects(catId);
      curatorialData = {
        wing: cat ? (cat.hall_num || `WING`) : 'WING',
        title: cat ? (isFa && cat.name_fa ? cat.name_fa : cat.name) : '',
        subtitle: cat ? (isFa && cat.subtitle_fa ? cat.subtitle_fa : cat.subtitle_en) : '',
        statement: cat ? (isFa && cat.curatorial_fa ? cat.curatorial_fa : cat.curatorial_en) : ''
      };
    }

    activeCategoryProjects = projects;

    // Update Curatorial Box
    if (collectionCuratorialBox && curatorialData) {
      collectionCuratorialBox.innerHTML = `
        <span class="curatorial-box-wing">${curatorialData.wing}</span>
        <h2 class="curatorial-box-title">${curatorialData.title}</h2>
        <p class="curatorial-box-sub">${curatorialData.subtitle}</p>
        <p class="curatorial-box-desc">${curatorialData.statement}</p>
      `;
    }

    // Render Masonry Cards
    const aspectClasses = ['aspect-tall', 'aspect-wide', 'aspect-square', 'aspect-tall', 'aspect-square', 'aspect-wide'];
    collectionMasonryGrid.innerHTML = projects.map((p, idx) => {
      const title = isFa && p.title_fa ? p.title_fa : p.title;
      const client = isFa && p.client_fa ? p.client_fa : p.client;
      const equipment = isFa && p.equipment_fa ? p.equipment_fa : (p.equipment || p.specs || 'Medium Format');
      const aspect = aspectClasses[idx % aspectClasses.length];
      const viewLabel = store.getTranslation('view_exhibition', currentLang) || (isFa ? 'مشاهده اثر' : 'VIEW EXHIBITION');

      return `
        <article class="masonry-card category-masonry-card ${aspect}" data-id="${p.id}" tabindex="0" role="button" aria-label="${title}">
          <img src="${p.cover}" alt="${title}" class="masonry-card-img" loading="lazy" referrerPolicy="no-referrer" />
          <div class="masonry-card-gradient"></div>
          
          <div class="masonry-card-badges">
            <span class="masonry-badge-client">${client}</span>
            <span class="masonry-badge-year">${p.year || '2025'}</span>
          </div>

          <div class="masonry-card-overlay">
            <div class="masonry-card-meta">
              <span class="masonry-card-specs"><i data-lucide="camera" style="width:12px; height:12px; display:inline-block; vertical-align:middle; margin-right:4px;"></i>${equipment}</span>
              <h3 class="masonry-card-title">${title}</h3>
            </div>
            <div class="masonry-card-action">
              <span>${viewLabel}</span>
              <div class="masonry-action-icon"><i data-lucide="${isFa ? 'arrow-left' : 'arrow-right'}"></i></div>
            </div>
          </div>
        </article>
      `;
    ...
    collectionMasonryGrid.querySelectorAll('.category-masonry-card').forEach(card => {
      card.addEventListener('click', () => {
    ...
    // Stagger animation
    if (window.gsap) {
      gsap.fromTo('.category-masonry-card', 
        { opacity: 0, y: 24 },
  ...
  // ==========================================
  // 7D. FEATURED WORKS TEASER (HOME / THE ATELIER)
  // 4 Curated Masterworks Preview
  // ==========================================
  const featuredTeaserGrid = document.getElementById('featured-teaser-grid');

  function initFeaturedTeaser() {
    if (!featuredTeaserGrid) return;
    const isFa = currentLang === 'fa';
    const allProjects = store.getAllProjects ? store.getAllProjects() : store.getProjects();
    const selectedIds = ['p-fashion-01', 'p-auto-01', 'p-arch-01', 'p-cosm-01'];
    let teaserProjects = selectedIds.map(id => store.getProjectById(id)).filter(Boolean);
    if (teaserProjects.length < 4) {
      teaserProjects = allProjects.slice(0, 4);
    }

    featuredTeaserGrid.innerHTML = teaserProjects.map((p, i) => {
      const title = isFa && p.title_fa ? p.title_fa : p.title;
      const client = isFa && p.client_fa ? p.client_fa : p.client;
      const category = isFa && p.category_fa ? p.category_fa : p.category;
      const specs = isFa && p.specs_fa ? p.specs_fa : p.specs;

      return `
        <article class="featured-teaser-card" data-id="${p.id}" tabindex="0" role="button" aria-label="${title}">
          <div class="featured-teaser-img-box">
            <img src="${p.cover}" alt="${title}" class="featured-teaser-img" loading="lazy" referrerPolicy="no-referrer" />
            <span class="featured-teaser-badge">MASTERWORK 0${i + 1} // ${category ? category.toUpperCase() : 'EXHIBITION'}</span>
          </div>
          <div class="featured-teaser-info">
            <div class="featured-teaser-meta">
              <span style="color: var(--accent-red); font-weight: 700; letter-spacing: 1px;">${client}</span>
              <span>${p.year || '2025'}</span>
            </div>
            <h3 class="featured-teaser-title">${title}</h3>
            <p style="font-size: 12px; color: var(--text-muted); margin-top: 4px;">✦ ${specs}</p>
          </div>
        </article>
      `;
    }).join('');

    featuredTeaserGrid.querySelectorAll('.featured-teaser-card').forEach(card => {
      card.addEventListener('click', () => {
        const id = card.getAttribute('data-id');
        if (id) {
          activeCategoryProjects = teaserProjects;
          openProjectModal(id);
        }
      });
    });
  ...
  window.refreshWorksLanguage = function() {
    ...
    if (collectionSidebarFilters && collectionMasonryGrid) {
      initCollectionPage();
    }
    if (featuredTeaserGrid) {
      initFeaturedTeaser();
    }
  ...
  // Initial render of Works / Teaser / Collection / Collective
  renderMuseumHalls();
  initCollectionPage();
  initFeaturedTeaser();

  // ==========================================
  // 8. COLLECTIVE CARDS & DOSSIER MODAL
  // ==========================================
  const collectiveContainer = document.getElementById('collective-container');
  ...
  function renderCollectiveCards() {
    if (!collectiveContainer) return;
    const members = store.getCollective();
    if (!members || !members.length) return;
    const isFa = currentLang === 'fa';
    const viewDossierLabel = store.getTranslation('view_dossier', currentLang) || (isFa ? 'مشاهده پرونده هنرمند' : 'VIEW ARTIST DOSSIER');

    collectiveContainer.innerHTML = members.map((member) => {
      ...
      const badge = member.id === 'amir-moghadam' 
        ? (isFa ? 'بنیان‌گذار و مدیر خلاقیت' : 'FOUNDER & CREATIVE DIRECTOR')
        : (isFa ? 'متخصص نورپردازی و ریتاچ تبلیغاتی' : 'LIGHTING & RETOUCHING SPECIALIST');

      return `
        <article class="collective-card" data-slug="${member.slug || member.id}">
          <div class="collective-image-box">
            <img src="${member.portrait}" alt="${name}" class="collective-img" loading="lazy" referrerPolicy="no-referrer" />
            <div class="project-overlay-gradient"></div>
            <div class="collective-badge">${badge}</div>
          </div>
          <div class="collective-details">
            <div class="collective-header">
              <h3 class="collective-name">${name}</h3>
              <span class="collective-role">${role}</span>
              <span class="collective-meta-pill"><i data-lucide="map-pin" style="width:12px; height:12px; display:inline-block; vertical-align:middle; margin-right:4px;"></i> ${born} • ${education}</span>
            </div>
            <p class="collective-bio">
              ${bio}
            </p>
            <div class="collective-expertise-list">
              ${expertise.map(exp => `<span class="expertise-tag">${exp}</span>`).join('')}
            </div>
            <div class="collective-action open-collective-dossier" data-slug="${member.slug || member.id}">
              <span>${viewDossierLabel}</span>
              <div class="action-arrow"><i data-lucide="${isFa ? 'arrow-up-left' : 'arrow-up-right'}"></i></div>
            </div>
          </div>
        </article>
      `;
    ...
    collectiveContainer.querySelectorAll('.collective-card, .open-collective-dossier').forEach(item => {
  ...
  window.renderCollectiveCards = renderCollectiveCards;
  ...
  function openCollectiveModal(slug) {
    ...
    collectiveModalBody.innerHTML = `
      ...
      <div style="background: rgba(255,255,255,0.02); padding: 20px; border-radius: var(--radius-sm); border: 1px solid var(--border-color); display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 16px;">
        ...
        <a href="collaborate.html" class="btn-primary" style="padding: 10px 20px; font-size: 11px;" onclick="document.getElementById('collective-modal').classList.add('hidden')">
